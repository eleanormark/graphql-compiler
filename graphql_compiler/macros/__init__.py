...
def get_schema_with_macros(macro_registry):
    ...
    for class_name, macros_for_class in six.iteritems(macro_registry.macro_edges_at_class):
        for macro_edge_name, macro_edge_descriptor in six.iteritems(macros_for_class):
            list_type_at_target = ListTypeNode(NamedTypeNode(NameNode(macro_edge_descriptor.target_class_name)))
            arguments = []
            directives = [DirectiveNode(NameNode(MacroEdgeDirective.name))]
            definitions_by_name[class_name].fields.append(FieldDefinitionNode(
                NameNode(macro_edge_name), arguments, list_type_at_target, directives=directives))
