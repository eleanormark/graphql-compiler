...
def validate_sqlalchemy_schema_info(schema_info):
    """Validate that the SQLAlchemySchemaInfo object is properly defined."""
    ...
    for type_name, graphql_type in six.iteritems(schema_info.schema.get_type_map()):
        if isinstance(graphql_type, types_to_map):
            if type_name != 'RootSchemaQuery' and not type_name.startswith('__'):
                ...
                if type_name not in schema_info.vertex_name_to_table:
                    raise AssertionError(u'Table for type {} not found'.format
                                         (type_name))
                table = schema_info.vertex_name_to_table[type_name]
                ...
                for field_name in six.iterkeys(graphql_type.fields):
                    if is_vertex_field_name(field_name):
                        if field_name not in schema_info.join_descriptors.get(type_name, {}):
